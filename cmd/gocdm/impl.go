...
// Run is a subcommand `gocdm`
// Flags:
//
//	dryRun: --dry-run -n (default: false) Dry run mode (emulate)
//	forceMenu: --menu -m (default: false) Force menu display
//	args: ... (min: 0) Arguments
func Run(dryRun bool, forceMenu bool, args ...string) {
	...
	case "C":
		...
		if dryRun {
			fmt.Printf("Dry run: Would exec console command: %s %v\n", bin, args)
			return
		}
	...
	case "X":
		...
		if dryRun {
			display = 0
			vt = "7"
			fmt.Printf("Dry run: Using dummy display :%d and vt%s\n", display, vt)
		} else {
			// Find free display
			...
			v, err := x11.GetVT(cfg.XTTY, display)
			if err != nil {
